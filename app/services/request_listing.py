...
from sqlalchemy import select, literal, case, union_all, func, or_
...
STAGES = ("pending", "pickup_pending", "awaiting_parcel", "parcel_received", "refund_approved",
          "gift_card_issued", "replacement_pending", "shipment_booked", "rejected")


def branch(model, kind):
    completed = (case((func.coalesce(model.gift_card_code, "") != "", "gift_card_issued"), else_="refund_approved")
                 if kind == "return" else
                 case((func.coalesce(model.outbound_tracking_id, "") != "", "shipment_booked"), else_="replacement_pending"))
    stage = case(
    ...
    return select(model.id.label("id"), literal(kind).label("kind"), model.created_at.label("created_at"),
                  ...
                  Customer.email.label("email"), Order.order_number.label("order_number")).select_from(model).join(
